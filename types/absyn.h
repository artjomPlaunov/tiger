...
struct A_exp_{
    ...
    union {A_var var;
        ...
        struct {A_exp test, then, else_;} if_; /* else_ is optional */
        struct {A_exp test, body;} while_;
        struct {S_symbol var; A_exp lo,hi,body; bool escape;} for_;
        /* break_; - need only the pos */
...
A_exp A_IfExp(A_pos pos, A_exp test, A_exp then, A_exp else_);
