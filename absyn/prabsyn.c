...
void pr_exp(FILE *out, A_exp v, int d) {
 ...
 case A_ifExp:
   fprintf(out, "iffExp(\n");
   pr_exp(out, v->u.iff.test, d+x); fprintf(out, ",\n");
   pr_exp(out, v->u.iff.then, d+x);
   if (v->u.iff.elsee) { /* else is optional */
      fprintf(out, ",\n");
      pr_exp(out, v->u.iff.elsee, d+x);
   }
   fprintf(out, ")");
   break;
 case A_whileExp:
   fprintf(out, "whileExp(\n");
   pr_exp(out, v->u.whilee.test, d+x); fprintf(out, ",\n");
   pr_exp(out, v->u.whilee.body, d+x); fprintf(out, ")\n");
   break;
 case A_forExp:
   fprintf(out, "forExp(%s,\n", S_name(v->u.forr.var)); 
   pr_exp(out, v->u.forr.lo, d+x); fprintf(out, ",\n");
   pr_exp(out, v->u.forr.hi, d+x); fprintf(out, "%s\n", ",");
   pr_exp(out, v->u.forr.body, d+x); fprintf(out, ",\n");
   indent(out, d+x); fprintf(out, "%s", v->u.forr.escape ? "TRUE)" : "FALSE)");
   break;
 case A_breakExp:
